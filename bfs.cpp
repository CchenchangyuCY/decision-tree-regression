...
struct treenode {
	...
	treenode(treenode * fat, int D) {
		...
		int tmp[(fa -> ranksize[d]) + 5];
		for(int i = 0; i < fa -> ranksize[d] + 5; i++) {
			tmp[i] = -1;
		}
		//vector<int> tmp((fa -> rank[d]).size() + 5, -1);
		for(int i = l; i <= r; i++) {
			tmp[fa -> rank[d][node_set[i]]] = node_set[i];
		}
		int cur = 0;
		for(int i = 0; i < fa -> ranksize[d]; i++) {
			if(tmp[i] == -1) {
				continue;
			}
		...
		while(!q.empty()) {
			...
			for(int d = 0; d < D; d++) {
		        ...
		        for (int i = l; i < r; i++) {
				    ...
				    if (newLoss < minLoss && newLoss < 0.9999999 * (u -> loss)) {
		                next_d = d;
		                next_cut = cut;
		                minLoss = newLoss;
					}
...
int main() {
	...
	for(int i = 0; i < D; i++) {
		...
		rootfa -> ranksize[i] = N;
		for(int j = 0; j < N; j++) {
			rootfa -> rank[i][tmp[i][j].second] = j;
